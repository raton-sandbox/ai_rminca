# SNAPSHOT DE CONTEXTO REORGANIZADO - PROYECTO RATÓN DE MINCA (v24.2.0)
## Estado de Éxito Actual
El cliente ha validado el diseño y funcionamiento del orquestador central. Este es el punto de retorno seguro.

## 1. Arquitectura de Archivos y Responsabilidades
* IA aplicada para el chat Groq
* 'raton.py': Orquestador central. Contiene las capas deterministas de intercepción local (Spam, Bots, Triviales), el formateador de respuestas de usuario, el canal Groq NLP (Llama-3.1-8b-instant con salida JSON estructurada) y la distribución analítica por tipos de flujo.
* 'core/geo_manager.py' ('CatalogoRutas'): Motor determinista de backend en Python y Pandas encargado de la carga de la base de conocimiento y normalización geográfica.
* 'handlers/': Módulos de aislamiento lógico ('origen_destino.py', 'interes_zona.py', 'perfil_filtro.py', 'zona_info.py', 'destino_puro.py').
* 'logger_aprendizaje.py': Persistencia y trazabilidad de interacciones para auditoría.

## 2. Código Fuente de Referencia Inalterable Orquestador (raton.py)
# -*- coding: utf-8 -*-
"""
...
## 3. Reglas de Negocio Validadas y Fijas
* Intercepción de saludos triviales localmente (0 tokens API, 0ms latencia).
* Mapeo semántico exhaustivo de intenciones (ej. "lugares para bañarme" -> tag "balnearios").
* Flujo peatonal/senderismo por defecto en trayectos punto a punto, a menos que se especifiquen términos de transporte vehicular explícitos.
* Monkey-patching defensivo incorporado en la cabecera para garantizar total compatibilidad con handlers heredados.
## 4. Objetivo final y framework
Incorporar el chatbot con IA  en el sitio web https://sites.google.com/site/ratondeminca
* EL sitio web esta desarrollado en el framework de google sites version 2016.
* Como el repositorio con los script de phyton  y los JSON con el conocimiento, se usara GIthub
*Para mantenener el servicio activo 24 horas,  se usara el Web Service ofrecido desde Render.com
* Todas las herramientas se usan en su  versión basica free (gratutita)