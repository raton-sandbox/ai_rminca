...
Versión: 24.2.0 - Trazabilidad Semántica Global y Handlers Homologados
Timestamp: 2026-07-05T14:11:00-05:00
"""
import os
import json
import re
...
def imprimir_error_handler(codigo_mensaje: str):
    ...
    print("Modifica tu petición o busca la sección correspondiente en el sitio web.\n")
...
def renderizar_respuesta_usuario(lista_rutas: list, interes_modo: str, prompt_original: str):
    ...
    try:
        if lista_rutas:
            ...
            urls_unicas = df_urls['url'].dropna().unique().tolist()
            urls_unicas = [u.strip() for u in urls_unicas if u.strip() != ""]
...
def procesar_con_ia_groq(texto_usuario: str) -> dict:
    prompt_sistema = (
        ...
        "Eres el motor de traducción semántica y clasificador de intenciones de 'Raton de Minca', un chatbot "
        "especializado en SENDERISMO, MONTAÑISMO y ECOTURISMO en la región de la Sierra Nevada de Santa Marta (Colombia).\n"
        "Tu ecosistema operativo son senderos, trochas, playas, ríos, cafetales y montañas "
        ...
        "##IMPORTANTE: La palabra 'senderos' o 'caminata' no es un interés válido en nuestra base de datos. Si el usuario la usa, debes leer entre líneas para descubrir si su interés real es el bosque, las aves, el rio, el balneario\n"
        ...
        "asume estrictamente que desea caminar, clasificando el modo como Peatonal (Flujo Senderismo) and asignando 'tipo_flujo': 'origen_destino'.\n"
        ...
        "EJEMPLOS DE CLASIFICACIÓN DE CONTROL (FEW-SHOT PROMPTING):\n\n"  
        "Ejemplo 1 (Palabras genéricas no son intereses):\n"
        "User: 'Muestrame senderos para llegar a pozo azul'\n"
        "Output: {\n"
        "  \"tipo_flujo\": \"zona_info\",\n"
        "  \"origen\": null,\n"
        "  \"destino\": null,\n"
        "  \"area\": \"pozo azul\",\n"
        "  \"interes\": null,\n"
        "  \"dificultad\": null,\n"
        "  \"duracion_deseada\": null\n"
        "}\n\n"
    
        "Ejemplo 2 (Interés explícito por sinónimo):\n"
        "User: 'Busco senderos en Guachaca en los que pueda nadar'\n"
        "Output: {\n"
        "  \"tipo_flujo\": \"interes_zona\",\n"
        "  \"origen\": null,\n"
        "  \"destino\": null,\n"
        "  \"area\": \"Guachaca\",\n"
        "  \"interes\": \"balnearios\",\n"
        ...
        "  \"duracion_deseada\": null\n"
        "}\n\n"
        "Ejemplo 3 (Trayecto puro sin interés vehicular):\n"
        "User: 'Caminatas o senderos desde Minca hasta el Campano'\n"
        "Output: {\n"
        "  \"tipo_flujo\": \"origen_destino\",\n"
        "  \"origen\": \"Minca\",\n"
        "  \"destino\": \"El Campano\",\n"
        "  \"area\": null,\n"
        "  \"interes\": null,\n"
...
def orquestar_pipeline(texto_usuario: str, metadata_red: dict = None):
    ...
    # =========================================================================
    # CAPA 1: FILTRO ANTI-SPAM, ATAQUES BOT O CONTROLES POR LONGITUD / REPETICIÓN
    # =========================================================================
    if len(texto_limpio) < 2 or len(texto_limpio) > 400:
        print("\n🛑 [ORQUESTADOR - INTERCEPCIÓN]: Petición descartada por anomalía en longitud (Spam/Bot).")
        print(f"💬 Mensaje inválido. Por favor, escribe una consulta de ruta o destino real en la Sierra Nevada.\n")
        return

    if re.search(r'(.)\1{4,}', texto_limpio) or not re.search(r'[a-zA-Z0-9]', texto_limpio):
        print("\n🛑 [ORQUESTADOR - INTERCEPCIÓN]: Petición bloqueada por patrón automatizado de Bot.")
        print(f"💬 Entrada bloqueada por patrón sospechoso. Asegúrate de redactar tu consulta en lenguaje natural.\n")
        return

    # =========================================================================
    # CAPA 2: INTERCEPCIÓN DETERMINISTA DE MENSAJES TRIVIALES (0 ms de coste de API)
    # =========================================================================
    GLOSARIO_TRIVIAL = {
        "hola": "¡Hola! Caminante. Bienvenido a Ratón de Minca. ¿Qué sendero, playa, cascada o balneario de la Sierra Nevada deseas consultar hoy?",
        "buenos dias": "¡Buenos días! Espero que el clima esté a favor de la trocha hoy. ¿Hacia dónde tienes planeado caminar?",
        "buenas tardes": "¡Buenas tardes! ¿Buscas información de transporte, rutas de senderismo o algún destino en particular?",
        "gracias": "¡Con todo el gusto! Si necesitas planear otra caminata o verificar cómo llegar a un lugar, aquí estaré. ¡Buenas rutas!",
        "muchas gracias": "¡De nada! Recuerda no dejar basura en los senderos y respetar el entorno natural. ¿Deseas consultar algo más?",
        "adios": "¡Buenas rutas! Camina con cuidado y regresa siempre con tus residuos. ¡Hasta pronto!",
        "chao": "¡Buenas rutas! Nos vemos en el próximo sendero.",
        "quien eres": "Soy el bot de 'Ratón de Minca', tu guía de conectividad y orientación para explorar la Sierra Nevada de Santa Marta."
    }

    entrada_normalizada = texto_limpio.lower().replace("?", "").replace("!", "").replace(",", "")
    if entrada_normalizada in GLOSARIO_TRIVIAL:
        print(f"\n⚡ [ORQUESTADOR - INTERCEPCIÓN LOCAL]")
        print(f" ├── Texto Original: '{texto_usuario}'")
        print(f" └── Tipo de Flujo : Trivial (Resuelto desde caché local RAM)")
        print(f"\n🤖 [BOTV02]: {GLOSARIO_TRIVIAL[entrada_normalizada]}\n")
        return

    # =========================================================================
    # CAPA 3: PROCESAMIENTO AVANZADO MEDIANTE LLM (MENSAJES RELEVANTES)
    # =========================================================================
    entidades = procesar_con_ia_groq(texto_limpio)
    ...
    if tipo_flujo == "zona_info":
        ...
        # CASO 2: Estructura combinada "Lugar en Área" (Ej: "Terrazas del Cacique en Guachaca")
            ...
            if not df_coincidencias.empty:
                ...
                print(f" ├── Sin intersección exacta en matriz. Delegando al handler de enlaces por zona...")
    ...
    # Flujo 2: Atractivos o intereses geolocalizados (interes_zona)
        ...
        area = entidades.get("area") or entidades.get("origen")
        if area and interes:
            es_positiva = detectar_asertividad(texto_limpio)
    ...
    # Flujo de Excepción / Desconocido
    else:
        print(f" ├── Componente Ejecutor: orquestar_pipeline -> flujo: DESCONOCIDO")
        print(f" └── Acción             : No se invocará ningún handler. Pasando directo al glosario de errores.")
        string_respuesta = "5"
